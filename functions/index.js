...
exports.sendPartnerOrganisationValidationEmailLive = functions.database.ref('/live/partnerOrganisation/{partnerId}')
  .onWrite(event => {
    ...
    admin.database().ref('live/userPublic/' + partnerOrganisation.userId).once("value", (data) => {
      let username = data.val().firstName + " " + data.val().lastName;
      let userEmail = data.val().email;

      admin.database().ref('live/agency/' + partnerOrganisation.agencyId).once("value", (agency) => {
        let agencyName = agency.val().name;

        admin.database().ref('live/countryOffice/' + partnerOrganisation.agencyId + '/' + partnerOrganisation.countryId).once("value", (country) => {
...
exports.sendNetworkAgencyValidationEmail_LIVE = functions.database.ref('/live/network/{networkId}/agencies/{agencyId}')
  .onWrite(event => {
    const preData = event.data.previous.val();
    const currData = event.data.current.val();
    ...
    if (!preData && currData) {
      ...
      admin.database().ref('/live/network/' + networkId).once("value", (data) => {
        ...
        if (data.val().isGlobal) {
          ...
          admin.database().ref('/live/agency/' + agencyId + '/adminId').once("value", (data) => {
            ...
            admin.database().ref('/live/userPublic/' + adminId).once("value", (user) => {
              ...
              admin.database().ref('live/networkAgencyValidation/' + agencyId + '/validationToken').set(validationToken).then(() => {
                console.log('success validationToken');
                const mailOptions = {
                  from: '"ALERT Network" <noreply@firebase.com>',
                  to: email
          ...
          console.log('isNotGlobal')
          admin.database().ref('/live/network/' + networkId + '/agencies/' + agencyId).once("value", (data) => {
            let countryOfficeCode = data.val().countryCode;
            admin.database().ref('/live/countryOffice/' + agencyId + '/' + countryOfficeCode + '/adminId').once("value", (data) => {
              let adminId = data.val();
              console.log("admin id: " + adminId);
              ...
              admin.database().ref('/live/userPublic/' + adminId).once("value", (user) => {
                ...
                admin.database().ref('live/networkAgencyValidation/' + countryOfficeCode + '/validationToken').set(validationToken).then(() => {
                  console.log('success validationToken');
                  const mailOptions = {
                    from: '"ALERT Network" <noreply@firebase.com>',
                    to: email
...
exports.createUserNetworkCountry_LIVE = functions.database.ref('/live/administratorNetworkCountry/{adminId}')
  .onWrite(event => {
    const preData = event.data.previous.val();
    const currData = event.data.current.val();
    ...
    if (!preData && currData) {
      ...
      let adminId = event.params['adminId'];
      console.log("admin id: " + adminId);
      admin.database().ref("/live/userPublic/" + adminId)
        .once("value", data => {
          let userDb = data.val();
          console.log(userDb);
...
exports.updateUserEmail_LIVE = functions.database.ref('/live/userPublic/{uid}/email')
  .onWrite(event => {
    const preData = event.data.previous.val();
    const currData = event.data.current.val();
...
exports.sendNetworkCountryAgencyValidationEmail_LIVE = functions.database.ref('/live/networkCountry/{networkId}/{networkCountryId}/agencyCountries/{agencyId}/{countryId}')
  .onWrite(event => {
    const preData = event.data.previous.val();
    const currData = event.data.current.val();
...
exports.sendEmailToExternalForAlertChange_LIVE = functions.database.ref('/live/alert/{countryId}/{alertId}/alertLevel')
  .onWrite(event => {
    const preData = event.data.previous.val();
    const currData = event.data.current.val();
    ...
    if (preData !== currData) {
      ...
      admin.database().ref('/live/alert/' + countryId + '/' + alertId).once("value", (data) => {
        let alert = data.val();
        console.log(alert);
        if (alert.hazardScenario !== -1) {
          admin.database().ref('/live/externalRecipient/' + countryId).once('value', (data) => {
            console.log("external recipient: ")
            console.log(data.val())
            let exObj = data.val();
            if (exObj) {
          ...
          admin.database().ref('/live/hazardOther/' + alert.otherName + "/name").once("value", (data) => {
            let otherHazardName = data.val()
            admin.database().ref('/live/externalRecipient/' + countryId).once('value', (data) => {
              let exObj = data.val();
              if (exObj) {
...
exports.sendEmailToExternalForAlertChangeRed_LIVE = functions.database.ref('/live/alert/{countryId}/{alertId}/approval/countryDirector/{directorId}')
  .onWrite(event => {
...
exports.sendEmailToExternalForIndicatorUpdate_LIVE = functions.database.ref('/live/indicator/{hazardId}/{indicatorId}/triggerSelected')
  .onWrite(event => {
    ...
    if (preData && currData !== preData) {
      ...
      admin.database().ref('/live/indicator/' + hazardId + '/' + indicatorId).once("value", (data) => {
        let indicator = data.val();
        if (indicator.hazardScenario['key'] === 'countryContext') {
          console.log("send email to state indicator for country context was updated")
          let title = `The indicator ${indicator.name} for Country Context has been updated`
          let content = `The following indicator: ${indicator.name} for Country Context has been updated`
          fetchUsersAndSendEmail('live', hazardId, title, content, UPDATE_HAZARD)
        } else {
          console.log("fetch country id for hazard")
          admin.database().ref('/live/hazard').once("value", (data) => {
            let filteredObjs = Object.keys(data.val()).map(key => {
              let obj = data.val()[key];
              obj['id'] = key
              return obj
            ...
            if (indicator.hazardScenario.hazardScenario !== -1) {
              let title = `The indicator ${indicator.name} for ${HAZARDS[indicator.hazardScenario.hazardScenario]} has been updated`
              let content = `The following indicator: ${indicator.name} for ${HAZARDS[indicator.hazardScenario.hazardScenario]} has been updated`
              fetchUsersAndSendEmail('live', countryId, title, content)
            } else {
              admin.database().ref('/live/hazardOther/' + indicator.hazardScenario.otherName + "/name").once("value", (data) => {
                let otherHazardName = data.val()
                let title = `The indicator ${indicator.name} for ${otherHazardName} has been updated`
                let content = `The following indicator: ${indicator.name} for ${otherHazardName} has been updated`
                fetchUsersAndSendEmail('live', countryId, title, content, UPDATE_HAZARD)
              })
            }
          })
        }
...
exports.sendEmailToExternalForPlanExpired_LIVE = functions.database.ref('/live/responsePlan/{countryId}/{planId}/isActive')
  .onWrite(event => {
    const preData = event.data.previous.val();
    const currData = event.data.current.val();
    ...
    if (preData && !currData) {
      ...
      console.log("response plan was expired")
      admin.database().ref('/live/responsePlan/' + countryId + '/' + planId).once('value', (data) => {
        let plan = data.val()
        console.log(plan)
        let title = `Plan ${plan.name} was expired`
        let content = `The following plan: ${plan.name} was expired.`
        fetchUsersAndSendEmail('live', countryId, title, content, PLAN_EXPIRED);
      })
    }
  })
...
exports.sendEmailPlanRejectedByCountryDirector_LIVE = functions.database.ref('/live/responsePlan/{countryId}/{planId}/approval/countryDirector/{countryDirectorId}')
  .onWrite(event => {
    ...
    if (currData === PLAN_NEEDREVIEWING) {
      ...
      admin.database().ref('/live/responsePlan/' + countryId + '/' + planId).once('value', (data) => {
        ...
        let title = `Response plan was rejected`
        let content = `The following response plan:${plan.name}, was rejected by country director.`
        fetchUsersAndSendEmail('live', countryId, title, content, PLAN_REJECTED)
      })
    }
  })
...
exports.sendEmailPlanRejectedByRegionDirector_LIVE = functions.database.ref('/live/responsePlan/{countryId}/{planId}/approval/regionDirector/{regionDirectorId}')
  .onWrite(event => {
...
exports.sendEmailPlanRejectedByGlobalDirector_LIVE = functions.database.ref('/live/responsePlan/{countryId}/{planId}/approval/globalDirector/{globalDirectorId}')
  .onWrite(event => {
