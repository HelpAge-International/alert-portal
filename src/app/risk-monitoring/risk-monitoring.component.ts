...
export class RiskMonitoringComponent implements OnInit {
    ...
    _getCountryContextIndicators() {
        let subscription = this.af.database.list(Constants.APP_STATUS + "/indicator/" + this.countryID).subscribe((indicators: any) => {
    ...
    _getHazards() {
        let promise = new Promise((res, rej) => {
            let subscription = this.af.database.list(Constants.APP_STATUS + "/hazard/" + this.countryID).subscribe((hazards: any) => {
                this.activeHazards = [];
