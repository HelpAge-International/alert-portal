...
export class PreparednessComponent implements OnInit, OnDestroy {
  ...
  getUsersForAssign() {
    ...
    this.af.database.object(Constants.APP_STATUS + "/staff/" + uid)
  ...
  _getPreparednessFrequency() {
    let promise = new Promise((res, rej) => {
      let subscription = this.af.database.object(Constants.APP_STATUS + "/countryOffice/" + this.agencyID + '/' + this.countryID + '/clockSettings/preparedness')
        .takeUntil(this.ngUnsubscribe)
