...
import {Department, ActionLevel, HazardCategory, DurationType} from "../../utils/Enums";
...
export class CreateEditPreparednessComponent implements OnInit, OnDestroy {
  ...
  _getPreparednessFrequency() {
    let promise = new Promise((res, rej) => {
      this.af.database.object(Constants.APP_STATUS + "/countryOffice/" + this.agencyID + '/' + this.countryID + '/clockSettings/preparedness')
        .takeUntil(this.ngUnsubscribe)
        .subscribe((frequencySetting: any) => {
