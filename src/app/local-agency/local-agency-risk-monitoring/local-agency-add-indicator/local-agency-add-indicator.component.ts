...
@Component({
  selector: 'app-local-agency-add-indicator',
  templateUrl: './local-agency-add-indicator.component.html',
  styleUrls: ['./local-agency-add-indicator.component.scss'],
  providers: [CommonService]
})
export class LocalAgencyAddIndicatorComponent implements OnInit {
      ...
      .subscribe((params: Params) => {
        ...
        this.pageControl.authUser(this.ngUnsubscribe, this.route, this.router, (user, userType, countryId, agencyId, systemId) => {
          ...
          this.userService.getAgencyDetail(this.agencyId)
            .first()
            .subscribe(agency => {
              this.agencyLocation = agency.country;
            });
        });
      });
  ...
  private loadCopyContextIndicatorInfo(copyAgencyId: string, copyIndicatorId: string, copyHazardId: string) {
    if (this.isContext && !copyHazardId) {
      this.af.database.object(Constants.APP_STATUS + "/indicator/" + copyAgencyId + "/" + copyIndicatorId)
        .takeUntil(this.ngUnsubscribe)
  ...
  addAnotherLocation() {
    ...
    modelArea.country = this.agencyLocation;
  ...
  getUsersForAssign() {
    if (this.UserType == UserType.Ert || this.UserType == UserType.PartnerUser) {
      ...
      this.af.database.object(Constants.APP_STATUS + "/agency/" + this.agencyId).subscribe((data: any) => {
        if (data.adminId) {
      ...
      this.af.database.object(Constants.APP_STATUS + "/staff/" + this.agencyId).subscribe((data: {}) => {
    ...
    this._validateData().then((isValid: boolean) => {
      if (isValid) {
        ...
        if (this.hazardID == 'countryContext') {
          urlToPush = Constants.APP_STATUS + '/indicator/' + this.agencyId;
          urlToEdit = Constants.APP_STATUS + '/indicator/' + this.agencyId + '/' + this.indicatorID;
        } else {
        ...
        if (!this.isEdit) {
          this.af.database.list(urlToPush)
            ...
            .then(indicator => {
              ...
              if (this.copySystemId && this.copyAgencyId) {
  ...
  _getHazards() {
    ...
    this.af.database.object(Constants.APP_STATUS + "/hazard/" + this.agencyId)
      .takeUntil(this.ngUnsubscribe)
      .subscribe((hazards: any) => {
  ...
  _deleteIndicator() {
    ...
    let path = "";
    if (this.hazardID == "countryContext") {
      path = Constants.APP_STATUS + "/indicator/" + this.agencyId + "/" + this.indicatorID;
    } else {
      path = Constants.APP_STATUS + "/indicator/" + this.hazardID + "/" + this.indicatorID;
    }
    this.af.database.object(path).set(null)
      .then(() => {
        this.router.navigateByUrl("/local-agency/risk-monitoring");
      })
      .catch((error) => {
  ...
  _getIndicator(hazardID: string, indicatorID: string) {
    ...
    if (this.hazardID == 'countryContext') {
      this.url = Constants.APP_STATUS + "/indicator/" + this.agencyId + '/' + indicatorID;
    } else {
