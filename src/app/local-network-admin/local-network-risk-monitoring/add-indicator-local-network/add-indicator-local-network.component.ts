...
@Component({
  selector: 'app-add-indicator-local-network',
  templateUrl: './add-indicator-local-network.component.html',
  styleUrls: ['./add-indicator-local-network.component.scss'],
  providers: [CommonService]
})
export class AddIndicatorLocalNetworkComponent implements OnInit, OnDestroy {
    ...
    this._validateData().then((isValid: boolean) => {
      if (isValid) {
        ...
        if(this.copyCountryOfficeCode && this.hazardID == 'countryContext'){
          urlToPush = Constants.APP_STATUS + "/indicator/" + this.networkId;
          urlToEdit = Constants.APP_STATUS + '/indicator/' + this.copyCountryOfficeCode + '/' + this.indicatorID;
        } else if (this.hazardID == 'countryContext') {
          urlToPush = Constants.APP_STATUS + '/indicator/' + this.networkId;
          urlToEdit = Constants.APP_STATUS + '/indicator/' + this.networkId + '/' + this.indicatorID;
        } else {
  ...
  _deleteIndicator() {
    ...
    if(this.copyCountryOfficeCode && this.hazardID == 'countryContext'){
      ...
      path = Constants.APP_STATUS + "/indicator/" + this.networkId + '/' + this.indicatorID;
    } else {
  ...
  _getIndicator(hazardID: string, indicatorID: string) {
    ...
    if(this.copyCountryOfficeCode && this.hazardID == 'countryContext'){
      ...
      this.url = Constants.APP_STATUS + "/indicator/" + this.networkId + '/' + indicatorID;
    } else {
