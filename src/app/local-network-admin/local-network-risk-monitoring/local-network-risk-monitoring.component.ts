...
export class LocalNetworkRiskMonitoringComponent implements OnInit, OnDestroy {
  ...
  updateHazardActiveStatus(modalID: string) {
    ...
    if (this.tmpHazardData['ID'] == "countryContext") {
      ...
      // Country Context
      this.markCountryContextArchived(this.networkId, this.tmpHazardData['activeStatus'], (error) => {
        if (error == null) {
  ...
  private getLocalNetworkAdmin(networkId: string) {
    this.networkService.getNetworkDetail(networkId).pipe(
      mergeMap(network => this.userService.getUser(network.networkAdminId)),
      takeUntil(this.ngUnsubscribe),)
